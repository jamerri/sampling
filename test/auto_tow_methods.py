...
for q in range(0, 5):
    D_train = []
    D_train = get_train_set(100, 9, 11, 3)
    print(len(D_train))
    '''设置测试集'''
    ...
    kz1 = 0.9  # 最优带宽参数
    for i in range(len(D_train)):
        train_position_x1.append(D_train[i].x)
        train_position_y1.append(D_train[i].y)
        train_c1.append(D_train[i].c)
    mean_value = fuc.mean_value_and_variance_value(train_position_x1, train_position_y1, train_c1, kz1)[0]
    variance_value = fuc.mean_value_and_variance_value(train_position_x1, train_position_y1, train_c1, kz1)[1]
    ...
    ws2 = 0.1  # 最优拉伸系数参数
    for i in range(len(D_train)):
        train_position_x2.append(D_train[i].x)
        train_position_y2.append(D_train[i].y)
        train_c2.append(D_train[i].c)
        train_wind_x2.append(D_train[i].wind_x)
        train_wind_y2.append(D_train[i].wind_y)
    calculate_speed = fuc.calculateSpeed_direction(train_wind_x2, train_wind_y2)[0]
    calculate_direction = fuc.calculateSpeed_direction(train_wind_x2, train_wind_y2)[1]
    mean_value = fuc.mean_value_and_variance_value_KernelDMVW(train_position_x2, train_position_y2, train_c2, calculate_speed, calculate_direction, kz2, ws2)[0]
    variance_value = fuc.mean_value_and_variance_value_KernelDMVW(train_position_x2, train_position_y2, train_c2, calculate_speed, calculate_direction, kz2, ws2)[1]
...
'''调用秩和检验'''
RES = fuc.signed_rank_test(NLPD1, NLPD2)
print(RES)
